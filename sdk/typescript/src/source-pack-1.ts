...
export type SourcePack1Category =
  | "general-vocabulary"
  | "morphology"
  | "entities"
  | "science-math"
...
export function* wiktionaryCandidates(
  records: Iterable<WiktionaryExtractRecord>,
...
export function* wikidataCandidates(
  records: Iterable<WikidataExtractRecord>,
): Generator<SourcePack1Candidate> {
  for (const record of records) {
    for (const [language, labelRaw] of Object.entries(record.labels)) {
      ...
      yield {
        ...
        semanticEvidence: record.descriptions?.[language]
          ? { description: record.descriptions[language] }
          : undefined,
