...
Do not build one monolithic file containing every source.

Recommended production flow:

```text
WordNet normalized candidates       ┐
Wiktionary normalized candidates    │
UniMorph candidates                 ├─> private alignment/sort -> corpus builder
Wikidata entity profile             │
Wikidata science/math profile       │
Wikidata programming profile        ┘
```

Each source can be updated independently. Stable private concept identities allow new evidence and aliases to enrich an existing concept without turning the corpus into a public dictionary.
